...
export default function AddFurnitureScreen({ navigation }) {
  ...
  function validate() {
    ...
    form.variants.forEach((v, index) => {
      const rowErrors = {};
      const hasAny = (v.name && v.name.trim()) || (v.value && v.value.trim()) || (v.price_adjustment !== undefined && String(v.price_adjustment).trim());
      if (hasAny) {
        ...
        if (v.price_adjustment !== undefined && String(v.price_adjustment).trim()) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const processedVariants = form.variants
        .filter((v) => v.name && v.name.trim() && v.value && v.value.trim())
        .map((v) => ({
          name: v.name.trim(),
          value: v.value.trim(),
          price_adjustment: v.price_adjustment ? parseFloat(sanitizeSignedNumeric(v.price_adjustment)) : 0,
        }));
      ...
      console.error('[AddFurnitureScreen] handleSave error:', err);
      const errMsg = err.message || err.details || String(err);
      Toast.show({
        type: 'error',
        text1: 'Failed to Add Furniture',
        text2: errMsg,
      });
    } finally {
      setLoading(false);
